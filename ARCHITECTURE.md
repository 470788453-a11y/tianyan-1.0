# 天衍1.0 / Architecture Notes

## 当前实现边界

这是一个独立 Phase 4 项目；当前开发计划正式命名为 **天衍1.0**。它不依赖 ClawCloud，也不和现有爪云页面耦合。

> 命名约定：对外计划名使用“天衍1.0”，内部目录/技术 slug 仍暂保留 `agentx-1.0`。
...
当前已落下第一步：受控命令执行（node/npm 版本探测）、artifact 落盘，以及 markdown/text 导出。
